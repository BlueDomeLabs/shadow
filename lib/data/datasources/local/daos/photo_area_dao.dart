...
class PhotoAreaDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<Result<void, AppError>> softDelete(
    ...
    try {
      ...
      final row =
          await (select(photoAreas)
                ..where((p) => p.id.equals(id) & p.syncDeletedAt.isNull()))
              .getSingleOrNull();
      if (row == null) return Failure(DatabaseError.notFound('PhotoArea', id));
      await (update(photoAreas)..where((p) => p.id.equals(id))).write(
        PhotoAreasCompanion(
  ...
  /// Mark a photo area as synced after successful cloud upload (AUDIT-03-001).
