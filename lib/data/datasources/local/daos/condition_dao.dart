...
class ConditionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<Result<void, AppError>> softDelete(
    ...
    try {
      ...
      final row =
          await (select(conditions)
                ..where((c) => c.id.equals(id) & c.syncDeletedAt.isNull()))
              .getSingleOrNull();
      if (row == null) return Failure(DatabaseError.notFound('Condition', id));
      await (update(conditions)..where((c) => c.id.equals(id))).write(
        ConditionsCompanion(
  ...
  /// Mark a condition as synced after successful cloud upload (AUDIT-03-001).
