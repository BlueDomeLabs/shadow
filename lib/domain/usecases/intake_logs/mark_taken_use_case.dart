// lib/domain/usecases/intake_logs/mark_taken_use_case.dart
...
/// Use case to mark an intake log as taken.
///
...
class MarkTakenUseCase implements UseCase<MarkTakenInput, IntakeLog> {
  final IntakeLogRepository _repository;
  final ProfileAuthorizationService _authService;

  MarkTakenUseCase(this._repository, this._authService);

  @override
  Future<Result<IntakeLog, AppError>> call(MarkTakenInput input) async {
    // 1. Authorization
    ...
    // 3. Validate actual time
    if (input.actualTime <= 0) {
      return Failure(
        ValidationError.fromFieldErrors({
          'actualTime': ['Actual time must be a valid epoch timestamp'],
        }),
      );
    }

    // 4. Repository call
    return _repository.markTaken(input.id, input.actualTime);
  }
}
