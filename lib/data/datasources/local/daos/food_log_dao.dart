...
class FoodLogDao extends DatabaseAccessor<AppDatabase> with _$FoodLogDaoMixin {
    ...
    try {
      ...
      final row =
          await (select(foodLogs)
                ..where((f) => f.id.equals(id) & f.syncDeletedAt.isNull()))
              .getSingleOrNull();
      if (row == null) return Failure(DatabaseError.notFound('FoodLog', id));
      await (update(foodLogs)..where((f) => f.id.equals(id))).write(
        FoodLogsCompanion(
  ...
  /// Mark a food log as synced after successful cloud upload (AUDIT-03-001).
  Future<Result<void, AppError>> markSynced(String id) async {
    try {
      ...
      return Failure(DatabaseError.updateFailed('food_logs', id, e, stack));
    }
  }
  ...
  List<String> _parseJsonList(String value) {
    ...
    try {
      ...
      debugPrint(
        'WARNING: Failed to parse JSON in FoodLogDao._parseJsonList: $e',
      );
      return [];
    }
  }
